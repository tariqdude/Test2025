...
export class AccessibilityAnalyzer implements AnalysisModule {
  ...
  async analyze(config: AnalyzerConfig): Promise<CodeIssue[]> {
    ...
    try {
      const files = await this.getProjectFiles(config, ['**/*.{astro,tsx,jsx,vue,svelte}']);
      ...
      logger.warn(`Accessibility analysis failed: ${analysisError.message}`, { error: analysisError });
    }
    return issues;
  }
  ...
  private async getProjectFiles(config: AnalyzerConfig, patterns: string[]): Promise<string[]> {
